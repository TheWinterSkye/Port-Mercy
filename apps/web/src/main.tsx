...
const rooms:Record<RoomId,RoomDef>={
 'southward.gas.forecourt':{id:'southward.gas.forecourt',name:'Mercy Fuel & Mart',district:'South Ward',desc:'Rain freckles the forecourt beneath washed-out canopy lights. The convenience store windows are caged in steel, and traffic from Harbor Avenue throws wet reflections across the pumps.',exits:{west:'southward.diner',east:'southward.alley',inside:'southward.apartment.lobby'},objects:[{id:'car1',label:'faded blue sedan',kind:'vehicle',x:.73,y:.72},{id:'npc1',label:'night clerk',kind:'npc',x:.29,y:.57}]},
 'southward.diner':{id:'southward.diner',name:"Rita's Diner",district:'South Ward',desc:'A narrow twenty-four-hour diner glowing red and cream through rain-streaked windows. The coffee is strong, the booths are split at the seams, and nobody asks why you are awake at this hour.',exits:{east:'southward.gas.forecourt'},objects:[{id:'npc2',label:'Rita Vale',kind:'npc',x:.66,y:.56}]},
 'southward.alley':{id:'southward.alley',name:'Mercy Service Alley',district:'South Ward',desc:'Wet brick walls squeeze around dumpsters, utility pipes and a fire escape. A security lamp buzzes overhead. Somewhere beyond the wall, an idling truck coughs and settles.',exits:{west:'southward.gas.forecourt'},objects:[{id:'prop1',label:'padlocked dumpster',kind:'prop',x:.58,y:.68}]},
 'southward.apartment.lobby':{id:'southward.apartment.lobby',name:'Marrow Apartments',district:'South Ward',desc:'Peeling green paint, dented brass mailboxes and an elevator that smells faintly of hot wiring. Apartment 3B is upstairs. The lobby camera over the door has not worked in years.',exits:{outside:'southward.gas.forecourt'},objects:[{id:'npc3',label:'tired tenant',kind:'npc',x:.61,y:.63}]}
};
...
function App(){
 ...
 return <main className="shell">
   ...
   <section className="center">
     ...
     <div className="commandBar"><span className="eyebrow">DO</span>{player.roomId==='southward.gas.forecourt'&&<><button onClick={work}>Unload delivery</button><button onClick={takeDrink}>Pocket energy drink</button><button onClick={openSedanTrunk}>Open sedan trunk</button><button onClick={()=>startTravel('southward.diner')}>Drive to Rita's</button></>}{player.roomId==='southward.diner'&&<><button onClick={work}>Wash dishes</button><button onClick={()=>push('Rita pours coffee without asking. “You look like you need this more than I need the quarter.”')}>Talk to Rita</button><button onClick={()=>push('The laminated menu is mostly breakfast, burgers and things that survived a fryer.')}>Read menu</button><button onClick={()=>startTravel('southward.gas.forecourt')}>Drive to Mercy Fuel</button></>}{player.roomId==='southward.alley'&&<><button onClick={()=>push('The dumpster padlock is cheap but intact. Someone has scratched a delivery code into the brick beside it.')}>Inspect dumpster</button></>}{player.roomId==='southward.apartment.lobby'&&<><button onClick={()=>push('Mailbox 3B contains a grocery circular and a final notice addressed to someone who lived here before you.')}>Check mailbox</button></>}</div>
   ...
   <footer><span>PORT MERCY // SOUTH WARD // NIGHT</span><span>v0.6.1 · {resourceCount?`${resourceCount} RESOURCES · `:''}WORLD HEARTBEAT {heartbeatLabel}</span></footer>
